...
import React, { useState, useRef, useEffect, useCallback, ReactNode } from 'react'; // Added ReactNode
...
const ScrollHighlightText: React.FC<ScrollHighlightTextProps> = ({
  ...
  const alignmentClass = styles[`textAlign${align.charAt(0).toUpperCase() + align.slice(1)}`];
  // Combine base, alignment, and any external className passed in props
  const combinedClassName = `${styles.highlightText} ${alignmentClass} ${className}`.trim();
